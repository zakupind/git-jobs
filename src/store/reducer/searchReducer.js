...
export default function searchReducer(state = stateSearch, action) {
  switch (action.type) {
    ...
    case NOT_RESULT_SEARCH:
      return {
        ...state, jobs: [], statusSearch: 'Вакансии по вашему запросу не найдены', isFetching: false,
      };
    case SUCCESS_SEARCH:
      return {
        ...state, jobs: action.payload, statusSearch: 'Результаты поиска', isFetching: false,
      };
